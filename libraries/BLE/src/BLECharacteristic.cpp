...
BLECharacteristic::BLECharacteristic(std::string uuid, uint32_t properties)
{
  ...
  if (uuid.length() == 4) {
    ...
    hexstr2bin(uuid.data(), (uint8_t *)&(BLEService::ble_service_char + BLEService::char_index)->char_uuid.uuid.uuid16,
               uuid.length() / 2);
  } else if (uuid.length() == 8) {
    (BLEService::ble_service_char + BLEService::char_index)->char_uuid.uuid_type = TKL_BLE_UUID_TYPE_32;
    hexstr2bin(uuid.data(), (uint8_t *)&(BLEService::ble_service_char + BLEService::char_index)->char_uuid.uuid.uuid32,
               uuid.length() / 2);
  } else if (uuid.length() == 32) {
    (BLEService::ble_service_char + BLEService::char_index)->char_uuid.uuid_type = TKL_BLE_UUID_TYPE_128;
    hexstr2bin(uuid.data(), (uint8_t *)&(BLEService::ble_service_char + BLEService::char_index)->char_uuid.uuid.uuid128,
               uuid.length() / 2);
  }
...
void BLECharacteristic::notify()
{
  if (notify_data_len != 0) {
    tkl_ble_gatts_value_notify(BLEDEV::ble_common_connect_handle,
                               (BLEService::ble_service_char + notify_handle)->handle, notify_data, notify_data_len);
  }
}
...
{
  if (notify_data_len != 0) {
    tkl_ble_gatts_value_indicate(BLEDEV::ble_common_connect_handle,
                                 (BLEService::ble_service_char + notify_handle)->handle, notify_data, notify_data_len);
  }
}

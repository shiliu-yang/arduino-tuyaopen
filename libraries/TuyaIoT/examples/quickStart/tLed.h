...
public:
   ...
   */
  void setState(LedState state, uint16_t blinkIntervalMs) { _setLedState(state, blinkIntervalMs); }

  /**
   * Sets the state of the LED using the previously set blink interval.
   * @param state The desired state of the LED (OFF, ON, BLINK).
   */
  void setState(LedState state) { _setLedState(state, _blinkIntervalMs); }
   ...
   */
  template <typename T, typename M> void setState(T state, M blinkIntervalMs)
  {
   ...
   */
  template <typename M> void blink(M ms)
  {
   ...
   */
  LedState getState() { return _ledState; }

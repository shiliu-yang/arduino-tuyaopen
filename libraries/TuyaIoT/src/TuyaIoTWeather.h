...
public:
  ...
  int getCurrentSunriseSunset(String &sunrise, String &sunset) { return getCurrentSunriseSunsetLocal(sunrise, sunset); }

  // Get current AQI
  int getCurrentAQI(int &aqi, int &qualityLevel, int &pm25, int &pm10, int &o3, int &no2, int &co, int &so2);
  int getCurrentAQICN(int &aqi, String &rank, int &qualityLevel, int &pm25, int &pm10, int &o3, int &no2, int &co,
                      int &so2); // rank only supports Mainland China

  // Get forecast weather conditions
  int getForecastConditions(int number, std::vector<int> &weather, std::vector<int> &temp, std::vector<int> &humi,
                            std::vector<int> &uvi, std::vector<int> &mbar);
  int getForecastConditionsCN(int number, std::vector<int> &weather, std::vector<int> &humi,
                              std::vector<int> &uvi); // Temp, pressure not support forecast in Mainland China
  ...
  /*
    ...
    you should guide users to allow their mobile app to access the location and pair the device with the mobile app
    again.
  */

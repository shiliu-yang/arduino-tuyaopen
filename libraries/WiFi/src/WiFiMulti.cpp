  ...
  if (scanResult == WIFI_SCAN_RUNNING) {
    ...
    if (scanResult == 0) {
      ...
      for (int8_t i = 0; i < scanResult; ++i) {
        ...
        if (known) {
          PR_INFO(" --->   %d: [%d][%02X:%02X:%02X:%02X:%02X:%02X] %s (%d) %c\r\n", i, chan_scan, BSSID_scan[0],
                  BSSID_scan[1], BSSID_scan[2], BSSID_scan[3], BSSID_scan[4], BSSID_scan[5], ssid_scan, rssi_scan,
                  (sec_scan == WAAM_OPEN) ? ' ' : '*');
        } else {
          PR_INFO("       %d: [%d][%02X:%02X:%02X:%02X:%02X:%02X] %s (%d) %c\r\n", i, chan_scan, BSSID_scan[0],
                  BSSID_scan[1], BSSID_scan[2], BSSID_scan[3], BSSID_scan[4], BSSID_scan[5], ssid_scan, rssi_scan,
    ...
    if (bestNetwork.ssid) {
      PR_INFO("[WIFI] Connecting BSSID: %02X:%02X:%02X:%02X:%02X:%02X SSID: %s Channel: %d (%d)\r\n", bestBSSID[0],
              bestBSSID[1], bestBSSID[2], bestBSSID[3], bestBSSID[4], bestBSSID[5], bestNetwork.ssid, bestChannel,
              bestNetworkDb);
      ...
      while (status != WSS_GOT_IP && status != WSS_NO_AP_FOUND && status != WSS_CONN_FAIL &&

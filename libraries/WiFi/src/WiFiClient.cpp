...
public:
  ...
  ~WiFiClientRxBuffer() { tal_free(_buffer); }
  ...
  size_t available() { return _fill - _pos + r_available(); }
...
public:
  ...
  ~WiFiClientSocketHandle() { tal_net_close(sockfd); }
...
#define ROE_WIFICLIENT(x, msg)                                                                            \
  {                                                                                                       \
    if (((x) < 0)) {                                                                                      \
      PR_ERR("Setsockopt '" msg "'' on fd %d failed. errno: %d, \"%s\"", sockfd, errno, strerror(errno)); \
      return 0;                                                                                           \
    }                                                                                                     \
  }
...
{
  return clientSocketHandle == rhs.clientSocketHandle && remotePort() == rhs.remotePort() &&

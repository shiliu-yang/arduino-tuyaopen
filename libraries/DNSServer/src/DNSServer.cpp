    ...
    if (requestIncludesOnlyOneQuestion()) {
      ...
      while (_buf[DNS_HEADER_SIZE + _DnsQuestion->QNameLength] != 0) {
        memcpy((void *)&_DnsQuestion->QName[_DnsQuestion->QNameLength],
               (void *)&_buf[DNS_HEADER_SIZE + _DnsQuestion->QNameLength],
               _buf[DNS_HEADER_SIZE + _DnsQuestion->QNameLength] + 1);
      ...
      memcpy(&_DnsQuestion->QType, (void *)&_buf[DNS_HEADER_SIZE + _DnsQuestion->QNameLength],
             sizeof(_DnsQuestion->QType));
      memcpy(&_DnsQuestion->QClass,
             (void *)&_buf[DNS_HEADER_SIZE + _DnsQuestion->QNameLength + sizeof(_DnsQuestion->QType)],
             sizeof(_DnsQuestion->QClass));
    }

    if (_DnsHeader->QRFlag == DNS_QR_QUERY && _DnsHeader->OPCode == DNS_OPCODE_QUERY &&
        requestIncludesOnlyOneQuestion() && (_domain == "*" || getDomainNameWithoutWwwPrefix() == _domain)) {
      replyWithIP();
...
{
  return ntohs(_DnsHeader->QuesCnt) == 1 && _DnsHeader->AnsCnt == 0 && _DnsHeader->AuthCnt == 0 &&
         _DnsHeader->ResCnt == 0;
}
...
void DNSServer::replyWithIP()
{
  ...
  uint16_t answerType = htons(DNS_TYPE_HOST_ADDRESS), answerClass = htons(DNS_CLASS_IN),
           answerIPv4 = htons(DNS_RDLENGTH_IPV4);
